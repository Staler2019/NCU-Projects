...
)					/* Move to zero only writes back dirty window */
{
	...
	printf("[move_window]move_window\n\r");
	wsect = fs->winsect;
	if (wsect != sector) {	/* Changed current window */
		...
		if (sector) {
			...
			{
				printf("[move_window]disk_read Failed\n\r");
				return FALSE;
			}
			fs->winsect = sector;
		}
	}
	printf("[move_window]move_window End\n\r");
	return TRUE;
}
...
)
{
	...
	for (;;) {
		...
		if (c == '\0' || c == '/') {		/* Reached to end of str or directory separator */
			...
			printf("[make_dirfile]make_dirfile = %d\n\r",c);
			return c;
		}
	...
	printf("[make_dirfile]make_dirfile Failed\n\r");
	return 1;
}
...
)
{
	...
	{					/* Null path means the root directory */
		*dir = NULL;
		printf("Null path means the root directory\n\r");
		return FR_OK;
	}
	...
	for (;;) {
		...
		if (ds == 1)
		{
			printf("[trace_path]FR_INVALID_NAME\n\r");
		...
		for (;;) {
			...
			{						/* Has it reached to end of dir? */
				printf("[trace_path]DIR_Name = %d=> FR_NO_FILE or FR_NO_PATH \n\r",dptr[DIR_Name]);
			...
			{
				printf("[trace_path]next_dir_entry => FR_NO_FILE or FR_NO_PATH \n\r");
				return !ds ? FR_NO_FILE : FR_NO_PATH;
			}
		}
...
)
{
	...
	{
		printf("disk_read(Load boot record) Failed\n\r");
		return 2;
	}
	if (LD_WORD(&fs->win[BS_55AA]) != 0xAA55)				/* Check record signature (always placed at offset 510 even if the sector size is >512) */
	{
		printf("BS_55AA Failed\n\r");
		return 2;
	}
	...
	printf("Valid boot record but not an FAT Failed\n\r");
	return 1;
}
...
)
{
	...
	*path = p;				/* Return pointer to the path name */
	...
	if (drv >= _DRIVES)
	{
		printf("drive number valid Failed\n\r");
	...
	*rfs = fs = FatFs[drv];					/* Returen pointer to the corresponding file system object */
	if (!fs)
	{
		printf("system object registered Failed\n\r");
	...
	{						/* If the logical drive has been mounted */
		...
		if (!(stat & STA_NOINIT)) {			/* and physical drive is kept initialized (has not been changed), */
			...
			{
				printf("write protection 1 Failed\n\r");
	...
	{
		printf("FR_NO_FILESYSTEM Failed\n\r");
	...
	if (chk_wp && (stat & STA_PROTECT))	/* Check write protection if needed */
	{
		printf("write protection 2 Failed\n\r");
		return FR_WRITE_PROTECTED;
	}
#endif
...
)
{
	...
	if (mode & (FA_CREATE_ALWAYS|FA_OPEN_ALWAYS|FA_CREATE_NEW)) {
		...
		if (res != FR_OK) {		/* No file, create new */
			if (res != FR_NO_FILE)
			{
				printf("FR_NO_FILE Failed\n\r");
				return res;
			}
			res = reserve_direntry(&dj, &dir);
			if (res != FR_OK)
			{
				printf("reserve_direntry Failed\n\r");
				return res;
			}
			...
			{
				printf("Cannot create new Failed\n\r");
				return FR_EXIST;
			}
			if (!dir || (dir[DIR_Attr] & (AM_RDO|AM_DIR)))	/* Cannot overwrite it (R/O or DIR) */
			{
				printf("Cannot overwrite it Failed\n\r");
				return FR_DENIED;
			}
	...
	/* Open an existing file */
		...
		{
			printf("It is a directory Failed\n\r");
			return FR_NO_FILE;
		}
#if !_FS_READONLY
		if ((mode & FA_WRITE) && (dir[DIR_Attr] & AM_RDO)) /* R/O violation */
		{
			printf("R/O violation Failed\n\r");
			return FR_DENIED;
		}
	}
...
#if _USE_STRFUNC >= 2
	if (chr == '\n') fputc_i ('\r', fil);	/* LF -> CRLF conversion */
#endif
...
)
{
	...
	for (n = 0; *str; str++, n++) {
		if (fputc_i(*str, fil) == EOF) return EOF;
	}
	return n;
}
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		if (c == 's') {				/* Type is string */
			cc = fputs_i(va_arg(arp, char*), fil);
			continue;
		}
		if (c == 'c') {				/* Type is character */
			cc = fputc_i(va_arg(arp, char), fil);
			if (cc != EOF) cc = 1;
			continue;
		}
		r = 0;
