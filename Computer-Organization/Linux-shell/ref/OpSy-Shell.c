// @ref: https://github.com/anandthegreat/OpSy-Shell
#include <ctype.h>
#include <fcntl.h>
#include <signal.h>
#include <stdio.h>
...
first inputExecution is called in main which manages std(out/err) redirection and then calls
inputExecutionSansStd inputExecutionSansStd breaks(splits) the input into different commands i.e.
p|q|r|s.. is broken into p,q,r,s... (for now piping isn't implemented)... p,q,r,s... are executed
individually in commandExecution.. commandExecution checks if its a normal command or if there is
any file redirection with the redirectionChecker irrespective of presence of redirection or not exec
is called
---------------------------------------------
*/
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(line, "\n") != 0) {
            sscanf(line, "%[^\n]%*c", input);       // get rid of necessory letter
            // scanf ("%[^\n]%*c", input);
            if (strcmp(input, "exit") == 0)         // exit
                break;

            else if (strstr(input, "cd") != NULL) { // cd
                dirChange(input);
            }
...
enum type stdRedirectionChecker(char *input, char **args) {  // checks which checks std redirection
    ...
    char *v =
        strstr(input, "2>&1");  // check if stderr redirection to stdout exists in the input string

    if (t != NULL && v != NULL)
        return STDOUT2FILE_AND_STDERR2STDOUT;
    else if (t != NULL && u != NULL)
        return STDOUT2FILE_AND_STDERR2FILE;
    else if (v != NULL)
        return STDERR2STDOUT;
    else if (t != NULL)
        return STDOUT2FILE;
    else if (u != NULL)
        return STDERR2FILE;
...
enum type redirectionChecker(char *input,
                             char **args) {  // checks if redirection exists, not std redirection
    ...
    if (p != NULL && r != NULL)
        return APPEND2FILE_READFROMFILE;  // should be above q!=NULL check
    else if (q != NULL && r != NULL)
        return OVERWRITE2FILE_READFROMFILE;
    else if (p != NULL)
        return APPEND2FILE;
    else if (q != NULL)
        return OVERWRITE2FILE;
    else if (r != NULL)
        return READFROMFILE;
    else
        return NORMAL;
...
void inputExecutionSansStd(char *input,
                           char **args) {  // input execution regardeless of std redirection
    char *commands[20];
    ...
    int numOfCommands = 0;
    while (commands[numOfCommands] != NULL) numOfCommands++;
...
void commandExecution(char *input,
                      char **args)  // executes every command based on redirection present
{
